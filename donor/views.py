...
def donor_list(request):
    location_filter = request.GET.get('location', '')  # Retrieve location from the query string
    if location_filter:
        donors = Donor.objects.filter(location__icontains=location_filter)  # Filter by location
    else:
        donors = Donor.objects.all()  # Show all donors if no filter

    context = {
        'donors': donors,
        'location_filter': location_filter,
    }
    return render(request, 'patient/donor_list.html', context)  # Use the patient's template
